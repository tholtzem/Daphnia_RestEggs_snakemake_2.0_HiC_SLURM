...
# load depth filters
if os.path.isfile("list/depthFilter.list"):
...
## realigned bam files LC without REF
bam_list = pd.read_csv("synthesis_daphnia/list/LC_realignedBAM_df1_withoutREF.list", sep='\t', index_col=False)
bams = list(bam_list['prefix'])
...
#BAM = list(prefixBAMdepth10['prefix'])##### helper functions ######
